...
def compute_accuracy_and_iou(result_blob, label_blob):
    ...
    for t in range(num_images):
        ...
        if np.max(result_labels) > max_label - 1:
            ...
            for class_id in range(0, max_label):
                ...
                tp[class_id] = tp[class_id] + np.count_nonzero(class_gt &
                                                               class_result)
                fp[class_id] = fp[class_id] + np.count_nonzero(class_result &
                                                               ~class_gt)
                fn[class_id] = fn[class_id] + np.count_nonzero(~class_result &
                                                               class_gt)
    ...
    for class_id in range(0, max_label):
        class_ious[class_id] = tp[class_id] / (tp[class_id] + fp[class_id] +
                                               fn[class_id])
